...
__forceinline bool AABBRayCollider::RayAABBOverlap(const Point& center, const Point& extents, Point& coord)
{
	...
	bool Inside = true;     // 379: no win32 stuff - was BOOL Inside = TRUE;

	// Find candidate planes.
	// 379: gcc uses ISO std - move decl of i out of loop
	udword i;
	for(i=0;i<3;i++)
	{
		if(mOrigin[i] < MinB[i])
		{
			coord[i]	= MinB[i];
			Inside		= false;   // 379: no win32 stuff - was Inside = FALSE
